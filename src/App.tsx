  ...
  return (
    <div className="w-full max-w-md mx-auto">
      <div className="flex justify-between text-sm text-indigo-200 mb-2">
        <span>
          {stage === "tracks" ? "Loading tracks" : "Fetching genres"}: {current}{" "}
          / {total}
        </span>
...
function App() {
  ...
  const fetchArtistGenres = useCallback(async (artists: Artist[]) => {
    const artistsWithoutGenres = artists.filter(
          ...
          trackObj.track.artists.forEach((artist) => {
            if (!artistMap[artist.id]) {
              artistMap[artist.id] = {
                ...
                savedCount: 0,
                genres: undefined,
                lastSaved: trackObj.added_at,
              };
            }
            artistMap[artist.id].savedCount += 1;
            if (
              !artistMap[artist.id].lastSaved ||
              new Date(trackObj.added_at) >
                new Date(artistMap[artist.id].lastSaved)
            ) {
  ...
  const handleClearCache = () => {
    if (
      window.confirm(
        "Are you sure you want to clear the cache? This will remove all stored artist data and you will need to reload from Spotify.",
      )
    ) {
  ...
  const columns = useMemo(
    () => [
      {
        accessorKey: "lastSaved" as const,
        header: () => "Last Saved in Library",
      ...
      },
      {
        ...
        header: () => "# Saved Tracks",
        cell: (info: CellContext<Artist, unknown>) => (
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-500/80 text-white border border-emerald-400/50">
      ...
      },
      {
        ...
        cell: (info: CellContext<Artist, unknown>) => (
          <button
            ...
            onClick={() =>
              window.open(
                `https://bandcamp.com/search?q=${encodeURIComponent('"' + info.row.original.name + '"')}&item_type=b`,
                "_blank",
              )
            }
          >
  ...
  const data = useMemo(() => {
    ...
    return allArtists
      ...
      .sort(
        (a, b) =>
          new Date(b.lastSaved || 0).getTime() -
          new Date(a.lastSaved || 0).getTime(),
      );
  ...
  const cacheAge = cacheTimestamp
    ? Math.round((Date.now() - parseInt(cacheTimestamp)) / (1000 * 60 * 60))
    : 0;
        ...
        <GlassCard className="p-8 mb-8">
          <div className="text-center">
            ...
            <p className="text-xl text-white/80 mb-6">
              Support the artists you love on Spotify by finding them on
              Bandcamp!
            </p>
            <p className="text-xl text-white/80 mb-6">
              Built by{" "}
                ...
                >
                  <GlassCard className="p-6">
                    ...
                    <p className="text-sm text-white/70 mb-4">
                      Copy the entire URL from the redirect page or just the
                      'code' parameter.
                    </p>
                    <form
          ...
          {authenticated && (
            <div className="flex flex-col items-center space-y-4">
              ...
              <p className="text-sm text-white/70">
                {cacheAge > 0 ? (
                  <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/10 border border-white/20">
